...
export default async function MarketingLayout({
  ...
  return (
    <>
      <SiteBanner />
      <SiteHeader />
      <main className="flex-1 px-[24px]">{children}</main>
      <SiteFooter />
      ...
      <div className="pointer-events-none absolute inset-0 h-24 w-full before:absolute before:bottom-[-20%] before:left-1/2 before:z-0 before:h-4/5 before:w-3/5 before:-translate-x-1/2 before:bg-[linear-gradient(90deg,hsl(264,100%,58%),hsl(226,100%,60%))] before:opacity-20 before:[filter:blur(calc(4*1rem))]" />
    </>
  )
}
